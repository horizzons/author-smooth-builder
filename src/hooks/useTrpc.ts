...
import { trpc, isTRPCClientError, getTRPCErrorType, getTRPCErrorMessage } from '@/utils/trpc';
...
/**
 * Transforms a tRPC error into a user-friendly error message and handles toast notifications
 */
export function handleTrpcError(error: unknown, showToast = true): ApiError {
  const { toast } = useToast();
  
  let apiError: ApiError;
  
  if (isTRPCClientError(error)) {
    const errorType = getTRPCErrorType(error);
    const errorMessage = getTRPCErrorMessage(error);
    
    apiError = new ApiError(errorMessage, errorType);
  } else if (error instanceof Error) {
    apiError = new ApiError(error.message, ApiErrorType.UNKNOWN);
  } else {
    apiError = new ApiError('An unknown error occurred', ApiErrorType.UNKNOWN);
  }
...
export function useTrpcQueryState<TData, TError>(query: {
  ...
  if (query.isError && query.error) {
    ...
    // Use React.useEffect here to prevent showing the toast on every render
    React.useEffect(() => {
      if (query.isError) {
        toast({
          title: 'Error',
